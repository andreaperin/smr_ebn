...
age_considered = 0.1;
N = 10^4; %number of montecarlo trials
HTE_dynamic = 0;
...
maint_effectiveness = 0.1; %it is actually 1-effectiveness
save_out = 0;
model = 'SMDFR';
All_Outputs = 0;
...
parfor i=1:N
    %%%%%%%%%%%%uncertain variables sampling (qui si può fare advanced
    %%%%%%%%%%%%MC)%%%%%%%%%
    model = 'SMDFR';
    age(i) = age_considered;
    ...
    ACS3_flow = 25;
    ACS4_flow = 25;
    if (LOCA1_time(i) < 1200) || (MSLB1(i) < 1200)
...
if saving
    save(fullfile(pwd, strcat('Input_single_reactor_',num2str(round(age_considered)))),'-v7.3');
end
