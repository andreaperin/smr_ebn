function p_fail = loca_failure_prob(age, PGA, params)
%LOCA_FAILURE_PROB Deterministic approximation of LOCA1 probability.
%   p_fail = LOCA_FAILURE_PROB(age, PGA, params)
%   age in years, PGA in m/s^2, params from default_params().LOCA

    p = params.LOCA;
    mu_corr = p.mu0 + corrosion_fun(age, p.Cr, p.dt, p.maint_effectiveness);
    sigma_corr = p.sigma0;
